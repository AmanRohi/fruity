...
const CartContainer = () => {
  ...
  const clearCart=()=>{
    ...
    querySnapshot.forEach((d) => {
        deleteDoc(doc(firestore,`cart@${user.email}`,d.id));
    });
    unsubscribe();
  });
  }
    ...
    const makePayment = async ()=>{
        const stripe = await loadStripe(env.PUBLISHER_KEY);
    ...
    flex-col z-[101]'>
       ...
       p-4'>
           ...
           <MdOutlineKeyboardBackspace className='text-textColor
            ...
            <motion.p onClick={clearCart}
            whileTap={{scale:0.75}} className='flex items-center gap-2 p-1 px-2 my-2 bg-gray-100
        ...
        <div className='w-full h-full bg-cartBg rounded-t-[2rem] flex flex-col'>
            ...
            flex flex-col items-center justify-evenly px-8 py-2'>
                ...
                <motion.button onClick={makePayment}
                whileTap={{scale:0.8}}
                type='button'
